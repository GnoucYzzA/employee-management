<template>
    <div class="edit-employee">
        <h1 class="EditEmployee__title">
            Edit {{ employee.name }} informations
        </h1>
        <el-form 
            :model="employee" 
            :rules="rules" 
...
<script>
    ...
    export default {
      ...
      created(){
        ...
        ref.doc(this.$route.params.employee_id).get().then(doc => {
            if(doc){
      ...
      methods: {
        onSubmit(formName) {
          this.$refs[formName].validate((valid) => {
            if (valid) {
                db.collection('employees').doc(this.employee.id).update({
